...
const PANE_STORAGE_KEY = 'siteforge.editor.panes.v6'
const RAIL_WIDTH = 44
const RESIZER_WIDTH = 8
const MIN_LEFT_PANEL = 150
const MIN_CANVAS = 720
const MIN_RIGHT_PANEL = 280
const DEFAULT_CANVAS_SHARE = 0.7
const DEFAULT_LEFT_SIDE_SHARE = 0.43
const DEFAULT_LEFT_PANEL = 260
...
watch(
  () => route.params.pageId,
  async (nextPageId) => {
    if (!nextPageId || nextPageId === pageId.value) return
    await loadRoutePage(nextPageId)
  }
)
...
function restorePaneLayout() {
  ...
  try {
    ...
    paneLayout.right = Number.isFinite(saved.right) ? clamp(saved.right, MIN_RIGHT_PANEL, 460) : defaults.right
  } catch {
...
function defaultPaneLayout() {
  ...
  const right = clamp(sideTotal - left, MIN_RIGHT_PANEL, 460)
  return { left, right }
}
...
async function openPage(targetPageId) {
  ...
  await loadRoutePage(targetPageId)
}

async function loadRoutePage(targetPageId) {
...
<style scoped>
.studio-editor {
  --studio-page: var(--sf-page, var(--sf-bg));
  --studio-header-line: var(--sf-header-line, var(--sf-outline-variant));
  --studio-header-bg: var(--sf-header-bg, color-mix(in srgb, var(--studio-page) 90%, var(--sf-surface) 10%));
  --studio-primary-soft: var(--sf-primary-soft, color-mix(in srgb, var(--sf-primary) 18%, transparent));
...
.studio-topbar {
  ...
  align-items: center;
  gap: 12px;
  border-bottom: 1px solid var(--studio-header-line);
